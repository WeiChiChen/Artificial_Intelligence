...
class GameState:
    ...
    def output_result(self):
        ...
        # output_file.write("root,0,-Infinity,-Infinity,Infinity\n")
...
def init_game_num():
    ...
    if tmp[0] == "X":
        ...
        x_num = -1
        o_num = 1
    state['depth'] = int(input_file.readline())
    ...
    for i in range(8):
        ...
        for j in range(8):
            if tmp[j] == "X":
                ...
                row.append(0)
        state['now_state'].append(row)
    state['prev_state'] = state['now_state'][:]
    input_file.close()
    # show_board(state)
    return state
...
def return_turn_player_number(gs, depth):
    num = 0
    # if gs.now_depth % 2 == 1 or gs.now_depth == 0:
    if depth % 2 == 1:
        # player's turn
        num = 1
    else:
...
def find_move(gs, depth):
    ...
    # TODO: can we check valid move and flip the board at the same time?
    for i in range(8):
        for j in range(8):
            if board[i][j] == 0:
                valid = False
...
def max_value(gs, state, depth, value, alpha, beta):
    ...
    # dump_node(gs)  # dump node
    # choose the max value move, first time call is root

    # alpha = gs.alpha
    # beta = gs.beta
    node = gs.node
    this_node_state = copy.deepcopy(state)
    ...
    if cut_off_test(gs, depth) or terminal_test(gs, depth):
        ...
        gs.value = eval_value
        # TODO: may not this update_state_and_log(gs, node, depth, eval_value, alpha, beta)
        log = [node, depth, eval_value, alpha, beta]  # value part!
        gs.log_track.append(log)
        # gs.now_depth = depth - 1
        return eval_value
    else:
        # root coming log
        if value == float("inf") or value == float("-inf"): value = float("-inf")  # for ths suspecion log
    ...
    if len(move_queue) == 0:
        ...
        if D_F: print "pass this turn", depth
        move_queue = ["pass"]
        """
        if not terminal_test(gs, depth):
            # TODO: update something
            if D_F: print "pass this turn", depth
            # gs.node = "pass"
            move_queue = ["pass"]
        else:  # terminal
            if D_F: print "terminal state"
            return eval_value
        """
    else:
        # clear pass flag
        set_pass_flag(depth, False)
    ...
    for step in move_queue:  # pick out best!
        ...
        value = min_value(gs, tmp_state, depth, value, alpha, beta)
        ...
        if value > alpha:
            alpha = value
            if depth == 1:  # because only next move will be do
            ...
            update_state_and_log(gs, node, depth - 1, alpha, alpha, beta)
...
def min_value(gs, state, depth, value, alpha, beta):
    ...
    if len(move_queue) == 0:
        # pass move
        set_pass_flag(depth, True)  # set pass flag
        if D_F: print "pass this turn", depth
        move_queue = ["pass"]
        """
        if not terminal_test(gs, depth):
            ...
            # gs.node = "pass"
            move_queue = ["pass"]
            # max_value(gs, this_node_state, depth, value, alpha, beta)
            # gs.value = eval_value
            # return eval_value
        else:  # terminal
            if D_F: print "terminal state"
    ...
    for step in move_queue:
        ...
        value = max_value(gs, tmp_state, depth, value, alpha, beta)
        ...
        if value < beta:
            ...
            update_state_and_log(gs, node, depth - 1, beta, alpha, beta)
